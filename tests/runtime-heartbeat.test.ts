...
describe('runtime heartbeat evidence', () => {
  ...
  test('does not attach existing connector rows without connector evidence', async () => {
    ...
    expect(calls.some((call) => call.sql.includes('INSERT INTO connector_instances'))).toBe(false)
    expect(calls.some((call) => call.sql.includes('UPDATE connector_instances'))).toBe(false)
