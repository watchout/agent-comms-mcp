...
function runBoundedGate(mutate:(x:any)=>void=()=>{}) {
  ...
  const handoff=ciAuthorityFixtures[1],od=ciAuthorityFixtures[2];
...
describe("bounded PR963 CI supply",()=>{
  ...
  test("A03/A05 canonical, ownership, scope, stale/conflicting consumer and metadata negatives",()=>{
    const cases:Array<[string,(x:any)=>void]>=[
      ...
      ["C1 consumer OD mixed with new I",x=>{x.fields.owner_decision_ref=x.controls[0].html_url;x.fields.owner_decision_body_sha256=createHash("sha256").update(x.controls[0].body).digest("hex")}],
      ["new OD missing",x=>x.controls.splice(2,1)],["normalization missing",x=>x.controls.splice(3,1)],
      ["predecessor missing",x=>x.controls.splice(4,1)],
      ["new OD hash",x=>x.controls[2].body+="changed"],["new OD actor",x=>x.controls[2].user.login="maker"],
      ["old OD id",x=>x.controls[0].id++],["old OD issue",x=>x.controls[0].issue_url=x.commentIssue],
      ["old OD association",x=>x.controls[0].author_association="MEMBER"],
      ["old OD wrong decision_id",x=>x.controls[0].body=x.controls[0].body.replace("OD-CTO-963-NARROW-CI-UNBLOCK-20260909-001","wrong")],
      ["fake old marker",x=>x.controls[0].body="<!-- shirube-v3:owner-decision:OD-CTO-963-NARROW-CI-UNBLOCK-20260909-001 -->\\n"+x.controls[0].body],
      ["new marker missing",x=>x.controls[2].body=x.controls[2].body.replace(/<!--[^>]*-->/,"")],
      ["new marker duplicate",x=>x.controls[2].body+="\\n"+x.controls[2].body.match(/<!--[^>]*-->/)[0]],
      ["normalization actor",x=>x.controls[3].user.login="maker"],
      ["normalization hash",x=>x.controls[3].body+="changed"],
      ["normalization marker",x=>x.controls[3].body=x.controls[3].body.replace(/<!--[^>]*-->/,"")],
      ["normalized object changed",x=>x.controls[1].body=x.controls[1].body.replace('normal_pushes: 2','normal_pushes: 3')],
      ["stale predecessor I as current",x=>{x.body=x.body.replace(x.controls[1].html_url,x.controls[4].html_url)}],
