...
export async function nativeHostFixture(home:string,workspace:string,agent:string,project:string,session:string,mode:'accepted'|'pending'|'absent'='accepted',runtimeId:string=randomUUID(), providerConfigRoot=home) {
  ...
  // A bounded CLI stand-in exposes the already connected native MCP transport.
  // Other commands are refused; this never invokes a provider or reads accounts.
  const cliPath=join(home,'native-config-bin');mkdirSync(cliPath)
  writeFileSync(join(providerConfigRoot,'native-fixture-transport.json'),JSON.stringify({enabled:true,transport:{type:'stdio',command:node,args:[memory],env:{...memoryEnv,FIXTURE_PROVIDER_PID:String(observed.provider.pid)}}}))
  const cli=join(cliPath,'codex')
  writeFileSync(cli, '#!'+node+'\n'+`import {readFileSync} from 'node:fs';import {join} from 'node:path';
    if(process.argv.slice(2).join(' ')==='mcp get wasurezu --json')process.stdout.write(readFileSync(join(process.env.CODEX_HOME,'native-fixture-transport.json'),'utf8'));
  ...
  // Capture only operation names/exit codes on observation failure, never process environments.
  let failedOperation:Record<string,unknown>|null=null
  const diagnosticObserver=createHostRuntimeObserver({
    run(command,args,timeout){
      try{return execFileSync(command,args,{encoding:'utf8',timeout,maxBuffer:16*1024*1024,
        stdio:['ignore','pipe','ignore'],env:{PATH:process.env.PATH!,LANG:'C',TZ:'UTC',LC_ALL:'C'}})}
      catch(error){failedOperation={command,args,status:(error as any).status,code:(error as any).code};throw error}
  ...
  const inspect:typeof inspectHostRuntime=input=>{
    failedOperation=null;const result=diagnosticObserver(input)
    if(result.reasonCode!=='OBSERVED')console.log(JSON.stringify({fixture:'native-host-observation',agent:input.agentId,expected_pid:observed.endpoint.pid,reason:result.reasonCode,failedOperation}))
    return result
  }
  return {runtimeId,cliPath,inspect,node,memory,env:{...memoryEnv,FIXTURE_PROVIDER_PID:String(observed.provider.pid)},observed,observeProvider}
}
