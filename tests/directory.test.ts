...
describe('bot/channel directory report', () => {
  test('combines agents, channels, role routing, and sendability', async () => {
    ...
    const historicalReport = await buildDirectoryReport(db, { includeHistorical: true })
    const aun = report.agents.find((agent) => agent.agent_id === 'codex-aun')
    const cto = report.agents.find((agent) => agent.agent_id === 'codex-cto')
    const legacy = historicalReport.agents.find((agent) => agent.agent_id === 'cto')
    const historicalCto = historicalReport.agents.find((agent) => agent.agent_id === 'codex-cto')
    ...
    expect(report.summary.agent_count).toBe(3)
    expect(report.agents.find((agent) => agent.agent_id === 'cto')).toBeUndefined()
    expect(historicalReport.summary.agent_count).toBe(4)
    expect(aun?.sendability).toBe('ready')
    expect(aun?.channels).toContain('agent-com')
    expect(legacy?.sendability).toBe('blocked')
    expect(legacy?.historical_only).toBe(true)
    expect(legacy?.new_work_allowed).toBe(false)
    expect(legacy?.warnings).toContain('disabled')
    expect(legacy?.warnings).toContain('historical_only')
    expect(legacy?.warnings).toContain('new_work_blocked')
    expect(legacy?.warnings).toContain('display_name_not_unique')
    expect(legacy?.warnings).toContain('discord_identity_not_unique')
    expect(cto?.warnings).not.toContain('discord_identity_not_unique')
    expect(historicalCto?.warnings).toContain('discord_identity_not_unique')
    ...
    expect(historicalReport.warnings).toContain('some_discord_identities_are_not_unique')
