...
test('NP07/08 normal daemon pending path re-reads real native original on private PostgreSQL; missing original/observer/lease dispatch zero', async () => {
  ...
  try {
    ...
    const calls: unknown[] = [], metrics = new FakeMetrics(), tmux = new FakeTmux(), alerts = new FakeAlertSink()
    const observerReasons:string[]=[]
    ...
    daemon = new StateDaemon({
      ...
      pgListen: new FakePgListen(), tmux, clock: new FakeClock(new Date()), metrics, alert: alerts,
      queueWorkScheduler: {async runPending(input) {calls.push(input)}},
      runtimeInspector: input => {inspections++; const result=observerAvailable ? inspectHostRuntime(input) : {reasonCode: 'OBSERVATION_TIMEOUT' as const, observations: []};observerReasons.push(result.reasonCode);return result},
    ...
    await dispatch()
    if(calls.length!==1)console.log(JSON.stringify({case:'scheduler-native-positive-denied',reads,inspections,observerReasons,metrics:metrics.calls,alerts:alerts.alerts,
      clock:new Date().toISOString(),evidence:await f.query('SELECT result_status,completed_at,valid_until FROM runtime_memory_ready_evidence WHERE agent_id=$1',[agent]),
      authority:await f.query('SELECT status,acquired_at,expires_at FROM control_plane_leases WHERE holder_agent_id=$1',[agent])}))
