...
test('NP04 actual managed server starts, releases while it owns the socket, and restarts with a new incarnation without physical DB writes', async () => {
  const f=await fixture('postgres',true), agent=`boot-${randomUUID()}`
  ...
  try {
    ...
    const start=async()=>{
      const p=Bun.spawn([join(dir,'codex'),'--no-env-file',join(dir,'provider.ts')],{cwd:dir,
        env:{PATH:process.env.PATH!,HOME:dir,TMPDIR:dir,LANG:'C',DATABASE_URL:url.href,AGENT_COM_DB:'postgres',
      ...
      let lease:any
      for(let i=0;i<100;i++) {
        lease=(await f.query("SELECT * FROM control_plane_leases WHERE holder_agent_id=$1 AND status='active'",[agent]))[0]
        if(lease)break
    ...
    const first=await start();await stop(first.p)
    ...
    console.log(JSON.stringify({case:'actual-server-lifecycle',starts:2,stops:2,restarts:1,recovered_claims:1,provider_invocations:0,external_sends:0,stderr:output}))
  } finally {
