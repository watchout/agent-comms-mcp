...
const STATE_DAEMON_CANARY_PREFLIGHT_PATH = join(REPO_ROOT, 'core', 'state-daemon-canary-preflight.ts')
...
const STATE_DAEMON_CANARY_PREFLIGHT_SRC = readFileSync(STATE_DAEMON_CANARY_PREFLIGHT_PATH, 'utf-8')
...
describe('T11e — NORM-060 full-channel smoke CLI surface', () => {
  ...
  test('help documents #742 state-daemon canary preflight evidence bundle', () => {
    expect(CLI_SRC).toMatch(/state-daemon canary-preflight --agent-id <id>/)
    expect(CLI_SRC).toMatch(/\[--require-scheduler-enabled\]/)
    expect(CLI_SRC).toMatch(/read-only #742 bounded #722 canary evidence bundle/)
    expect(STATE_DAEMON_CANARY_PREFLIGHT_SRC).toMatch(/issue_ref: '#742'/)
    expect(STATE_DAEMON_CANARY_PREFLIGHT_SRC).toMatch(/queue_daemon_status_not_used_as_evidence: true/)
    expect(STATE_DAEMON_CANARY_PREFLIGHT_SRC).toMatch(/no_live_canary_insert: true/)
    expect(STATE_DAEMON_CANARY_PREFLIGHT_SRC).toMatch(/no_state_daemon_restart: true/)
    expect(STATE_DAEMON_CANARY_PREFLIGHT_SRC).toMatch(/no_launchctl_mutation: true/)
