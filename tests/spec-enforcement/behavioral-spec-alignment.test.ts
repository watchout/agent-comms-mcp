...
const SEND_FALLBACK_SRC = readFileSync(join(REPO_ROOT, 'core', 'send-fallback-decision.ts'), 'utf-8')
...
describe('Behavioral FAIL B2 — MCP send per-row claim guard (Issue #278 segment 3a)', () => {
  test('send handler refuses missing/closed claims before projection (Issue #580)', () => {
    ...
    expect(handler).toMatch(/Error \[CLAIM_EXPIRED\]/)
    ...
    const expiredIdx = handler.indexOf('Error [CLAIM_EXPIRED]')
  ...
  test('expired received/in_progress claims are not active and fail before projection writes', () => {
