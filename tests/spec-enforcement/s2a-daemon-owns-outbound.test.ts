...
describe('S2-A (FEAT-005) — daemon-owns-outbound', () => {
  ...
  test('10. restart-bot.sh skips Codex update prompts without submitting the normal start screen', () => {
    ...
    expect(step5).toContain('[ "$RUNTIME_ENGINE" = "codex" ]')
  ...
  test('10c. restart-bot.sh requires the DB bot profile and has no registry fallback', () => {
    ...
    expect(script).not.toContain('runtime_engine_preference')
    expect(script).toContain('resolveSeatProvider')
    expect(script).toContain('allowHistory:true')
    expect(script).toContain('PROFILE_SOURCE="agents.profile"')
    expect(script).toContain("agent_type NOT IN ('human', 'system')")
    expect(script).toContain("buildStartLaunchArgv")
    ...
    expect(script).toContain('DB profile query failed')
    expect(script).toContain('await start({agentId:process.argv[2],runtime:process.argv[3]')
  ...
  test('10d. restart-bot.sh runs startup safety preflight before tmux or port mutation', () => {
    ...
    expect(cleanupIdx).toBe(-1) // A prior numeric port never grants cleanup authority.
  ...
  test('11. watchdog.sh delegates to the bounded canonical runtime observer', () => {
    const script = readFileSync(join(REPO_ROOT, 'scripts', 'watchdog.sh'), 'utf-8')
    expect(script).toContain('${SCRIPT_DIR}/../bin/aun-watchdog.ts" --once')
    expect(script).not.toContain('FROM agents')
    expect(script).not.toContain('runtime_engine_preference')
    expect(script).not.toContain('BOT_REGISTRY')
    expect(script).not.toContain('send-keys')
    expect(script).not.toContain('kill-session')
    const observer = readFileSync(join(REPO_ROOT, 'bin', 'aun-watchdog.ts'), 'utf-8')
    expect(observer).toContain('resolveRuntimeEndpoint')
    expect(observer).toContain('runtimeEndpointVerified')
    expect(observer).toContain('profile_enabled')
    expect(observer).toContain('disabled_at')
  })
  ...
  test('13b. MCP lifecycle bot inventory reads DB bot profiles only', () => {
    ...
    expect(loader).toContain("agent_type NOT IN ('human', 'system')")
    expect(loader).toContain('COALESCE(profile_enabled, true)')
    expect(loader).toContain('disabled_at IS NULL')
    ...
    expect(loader).not.toContain("agent_type <> 'human'")
  ...
  test('13bb. MCP lifecycle restart sends Codex commands literally and avoids normal-screen Enter', () => {
    ...
    expect(fn).toContain('buildStartLaunchArgv(plan)')
    expect(fn).toContain("tmuxExec(['send-keys', '-t', tmuxTarget, '-l', command])")
  ...
  test('13c. wake-daemon session resolvers use the enabled DB bot profile predicate', () => {
    const wakeDaemon = readFileSync(join(REPO_ROOT, 'bin', 'wake-daemon.ts'), 'utf-8')
    const pgResolver = sliceFn(wakeDaemon, 'resolvePgProfileSession')
