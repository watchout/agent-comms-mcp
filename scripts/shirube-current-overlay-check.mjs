...
// binds the complete reviewed canonical object, including all 124 admitted integration paths. No remote event budget is inferred.
async function requireBoundedCiSupply() {
  ...
  const currentC5="09d1b5e38ab3f6f16af9e85332935ad95bfd90c8";
  ...
  check(ref===`https://github.com/${target}/issues/940#issuecomment-5658354436`
    &&digest==="a018146fd84f020908bfcf1eddfd8609232e3fbdb5d339c854af42b132dc1916","exact published I required");
  ...
  const marker="<!-- shirube-v3:control-handoff:CH-CTO-AUN-POC-INTEGRATION-20260914-I6 -->";
  ...
  check(handoff.schema_version==="shirube-control-handoff/v1"
    &&handoff.handoff_id==="CH-CTO-AUN-POC-INTEGRATION-20260914-I6"
    &&handoff.subject.repository===target&&handoff.subject.pr===963
    &&handoff.subject.current_public_head===currentC5&&handoff.subject.base===base
    ...
    &&handoff.bounds.new_candidates===1&&handoff.bounds.cumulative_candidate_limit===6
    &&handoff.bounds.new_full_suite_runs===1&&handoff.bounds.cumulative_full_suite_limit===7
    ...
    &&handoff.bounds.two_callers_consumed===20&&handoff.bounds.owned_ci_subject_diagnostics===1
    &&handoff.bounds.two_callers_additional_local_specimens===1&&handoff.bounds.two_callers_cumulative_limit===21
    &&handoff.subject.current_local_head===currentC5
    ...
    &&handoff.allowed_paths.length===124&&new Set(handoff.allowed_paths).size===124,"current integration supply mismatch");
  ...
  git("merge-base","--is-ancestor",origin,currentC5);
  git("merge-base","--is-ancestor",currentC5,headSha);
  ...
  const repairs=[".github/workflows/pr-checks.yml",
  ...
  check(JSON.stringify([...handoff.repair_paths].sort())===JSON.stringify(repairs.sort())
    &&git("diff","--no-renames","--name-only",`${handoff.subject.current_local_head}...${headSha}`).split("\n").filter(Boolean)
      .every(file=>repairs.includes(file)),"candidate repair outside current I6 scope");
