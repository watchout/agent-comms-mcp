...
retired STATE_DAEMON_AGENT_DENYLIST is no longer rendered: eligibility is
DB-only (agents table + agent_type human guard, Issue #940 fix 3).
`
}
...
export async function main(
  ...
  try {
    ...
    if (!repoRoot || !databaseUrl) throw new Error('--repo-root and --database-url are required')
    const rendered = renderMemoryReadyRefreshLaunchAgent({ repoRoot, bunPath, databaseUrl, logRoot })
