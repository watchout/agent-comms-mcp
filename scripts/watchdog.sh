...
PSQL_BIN="${AUN_WATCHDOG_PSQL_BIN:-psql}"
LOG_TAG="[watchdog]"
...
PROFILE_ROWS=""
if ! PROFILE_ROWS="$("$PSQL_BIN" "$DATABASE_URL" -X -q -t -A -F '|' -v ON_ERROR_STOP=1 <<'SQL'
SELECT
...
SQL
  ...
  echo "${LOG_TAG} DB bot profile query failed; refusing to report 0/0 healthy" >&2
  exit 1
fi
...
while IFS='|' read -r SESSION PROJECT_DIR AGENT_ID PORT RUNTIME_ENGINE; do
  ...
  # --- Check 2: crash pattern detection DISABLED ---
  # Pane scrollback can contain ordinary words like panic/fatal/killed from
  # conversation or build output. Do not grep output for crash words here.
  ...
  # --- Check 3b: port liveness verification with 60s grace period ---
  if [ -n "${PORT:-}" ]; then
