...
CREATE OR REPLACE FUNCTION aun_configuration_legacy_desired_document(input agents)
RETURNS JSONB
LANGUAGE SQL
IMMUTABLE
STRICT
...
CREATE OR REPLACE FUNCTION aun_configuration_legacy_complete(input agents)
RETURNS BOOLEAN
LANGUAGE SQL
IMMUTABLE
STRICT
...
-- Stable seat policy excludes host/provider/endpoint diagnostics. No runtime values are rewritten.
...
AS $$
  SELECT jsonb_build_object(
    ...
    'expected_provider_identity_ref', input.expected_provider_identity_ref,
    'ordinary_communication_enrollment', input.ordinary_communication_enrollment,
    'ordinary_projection', input.ordinary_projection - ARRAY['provider_repo_root', 'provider_config_root', 'daemon_checkout']::text[],
    'profile_enabled', input.profile_enabled,
    'provider_token_source_ref', input.provider_token_source_ref,
    'release_commit', input.desired_release_commit,
    'release_tree', input.desired_release_tree,
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.desired_revision IS NOT NULL
     AND OLD.desired_digest IS DISTINCT FROM encode(digest(convert_to(aun_canonical_jsonb(aun_configuration_desired_document(OLD)), 'UTF8'), 'sha256'), 'hex')
     AND OLD.desired_digest IS DISTINCT FROM encode(digest(convert_to(aun_canonical_jsonb(aun_configuration_legacy_desired_document(OLD)), 'UTF8'), 'sha256'), 'hex') THEN
    RAISE EXCEPTION 'AUN_DESIRED_PRIOR_DIGEST_INVALID';
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND aun_configuration_complete(OLD)
     AND aun_configuration_desired_document(OLD) IS NOT DISTINCT FROM aun_configuration_desired_document(NEW)
     AND OLD.desired_digest IS NOT DISTINCT FROM next_digest THEN
...
-- Only verified complete legacy documents change format. Invalid/incomplete
-- history is preserved verbatim and remains unavailable to strict readers.
-- The existing trigger supplies revision/provenance and exactly one outbox event.
UPDATE agents a SET profile_enabled = a.profile_enabled
 WHERE a.desired_revision > 0
   AND aun_configuration_legacy_complete(a)
   AND a.desired_digest = encode(digest(convert_to(aun_canonical_jsonb(aun_configuration_legacy_desired_document(a)), 'UTF8'), 'sha256'), 'hex')
   AND a.desired_digest IS DISTINCT FROM encode(digest(convert_to(aun_canonical_jsonb(aun_configuration_desired_document(a)), 'UTF8'), 'sha256'), 'hex');
COMMIT;
