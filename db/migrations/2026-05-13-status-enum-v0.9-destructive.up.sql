...
-- the legacy literals):
--
--   1. audit_log table (idempotent CREATE)        -- §1.1a sink
...
-- 1. audit_log table for §1.1a 3-way branch + §1.1b skipped drop sink.
...
CREATE INDEX IF NOT EXISTS idx_audit_log_queue_id ON audit_log(queue_id);

-- 2. ADD done_at column (§1.1 schema). Idempotent.
ALTER TABLE message_queue ADD COLUMN IF NOT EXISTS done_at TIMESTAMPTZ;

-- 3. Data migration (§1.1b mapping). Must run before the CHECK constraint
--    swap because Postgres validates ADD CONSTRAINT against existing rows.
DO $$
DECLARE
...
-- 4. CHECK constraint swap (§1.1 schema). All rows are now in the new
--    enum after step 3, so the ADD CONSTRAINT validation pass succeeds.
DO $$
BEGIN
  IF EXISTS (
...
--    surviving rows have failed_reason = NULL and the audit_log holds
