...
async function migrate() {
  ...
  await client.query(`
    ...
    -- Issue #323 / PR #338 — v0.9 state machine migration (spec §1.1, §1.1a).
    -- Replaces the v0.8 enum (pending / read / replied / skipped / failed) with
    -- the v0.9 enum (pending / received / in_progress / done / replied). The
    -- failed status and the failed_reason column are retired by the
    -- transparent retry loop owned by state-daemon. Migration is idempotent:
    --   - audit_log table created IF NOT EXISTS
    --   - done_at column added IF NOT EXISTS
    --   - data transform only fires when v0.8 statuses are still present
    --     (no-op once the CHECK constraint has been swapped)
    --   - CHECK constraint swap is DO-guarded against duplicate runs
    --   - failed_reason DROP COLUMN IF EXISTS

    -- Phase 0: queue_audit_archive table (per §1.1a). Preserves the original
    -- failed_reason of rows that get terminally closed as replied, so operators
    -- can still reconstruct cause without retaining the column on the hot
    -- table. Named queue_audit_archive (not audit_log) because the existing
    -- audit_log table is the generic event-log used by access.denied etc;
    -- conflating schemas was a §1.1a anti-pattern caught here.
    CREATE TABLE IF NOT EXISTS queue_audit_archive (
      id BIGSERIAL PRIMARY KEY,
      queue_id BIGINT,
    ...
    CREATE INDEX IF NOT EXISTS idx_queue_audit_archive_queue_id ON queue_audit_archive(queue_id);
    CREATE INDEX IF NOT EXISTS idx_queue_audit_archive_archived_at ON queue_audit_archive(archived_at);

    -- Phase 1: new column for the in_progress→done→replied transition.
    ALTER TABLE message_queue ADD COLUMN IF NOT EXISTS done_at TIMESTAMPTZ;

    -- Phase 2a: CHECK constraint widened to accept BOTH the v0.8 statuses
    -- (read/skipped/failed) AND the v0.9 statuses (received/in_progress/done)
    -- simultaneously while the data transform runs. Without this widening
    -- the transform itself violates the v0.8 CHECK the moment it writes a
    -- v0.9 status. The narrow v0.9-only CHECK is installed in Phase 3 once
    -- no v0.8 row remains.
    DO $$
    BEGIN
      IF EXISTS (
    ...
    -- Phase 2b: data transform — fires only when v0.8 rows still exist. The
    -- transform follows the §1.1a 3-way branch contract.
    DO $$
    BEGIN
      IF EXISTS (
        ...
        -- Branch A — recent IMPLICIT_ABANDON → reset to pending (retry loop reclaims).
        UPDATE message_queue
           SET status = 'pending', failed_reason = NULL
         WHERE status = 'failed'
           AND failed_reason = 'IMPLICIT_ABANDON'
           AND claim_expires_at IS NOT NULL
           AND claim_expires_at > now() - interval '60 seconds';

        -- Branch B — STALE_DISPATCH → conservative reset (operator review path).
        UPDATE message_queue
           SET status = 'pending', failed_reason = NULL
         WHERE status = 'failed'
           AND failed_reason = 'STALE_DISPATCH';

        -- Branch C — remaining failed rows go to terminal close as replied,
        -- with original reason archived to audit_log so post-merge audits
        -- can still trace cause.
        INSERT INTO queue_audit_archive (queue_id, original_status, original_reason, note)
          SELECT id, 'failed', failed_reason, 'v0.9 migration — failed → replied'
            FROM message_queue
           WHERE status = 'failed';
        UPDATE message_queue
           SET status = 'replied', failed_reason = NULL
         WHERE status = 'failed';

        -- v0.8 read goes to v0.9 received (in-flight claims preserved).
        UPDATE message_queue
           SET status = 'received'
         WHERE status = 'read';

        -- v0.8 skipped rows are archived + DELETEd. skipped rows were the
        -- CC-fanout ghosts that v0.9 abolishes; nothing reads them post-migration.
        INSERT INTO queue_audit_archive (queue_id, original_status, original_reason, note)
          SELECT id, 'skipped', failed_reason, 'v0.9 migration — skipped → DELETE'
            FROM message_queue
           WHERE status = 'skipped';
        DELETE FROM message_queue WHERE status = 'skipped';
      END IF;
    END $$;

    -- Phase 3: CHECK constraint swap to the v0.9 enum. Idempotent — only
    -- DROP/ADD when the live constraint definition is missing one of the
    -- new statuses (signals it is still the v0.8 shape).
    DO $$
    BEGIN
      IF EXISTS (
        SELECT 1
    ...
    -- Phase 4: retire the failed_reason column. The transparent retry loop
    -- (state-daemon) keeps reason out of the hot table; archived rows live in
    -- audit_log. IF EXISTS so re-runs become no-ops.
