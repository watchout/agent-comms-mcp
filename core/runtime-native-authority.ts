...
export async function resolveNativeRuntimeAuthority(db:LogicalDb,input:{agentId:string;inspect?:NativeHostRuntimeInspector}):Promise<NativeRuntimeAuthority> {
  ...
  const read=async()=>{
    const result=await db.query(`SELECT r.runtime_instance_id,r.agent_id,r.commit_sha,r.metadata,
      l.lease_id,l.holder_agent_id,l.holder_runtime_instance_id,l.fencing_token,l.acquired_at,
  ...
  try {
    ...
    if(observed.agent_id!==input.agentId || observed.runtime_instance_id!==String(row.runtime_instance_id)
      || !authorityAcquiredAfterStart(row.acquired_at,observed.process_started_at))return fail()
    ...
    if(object(current.metadata).source_commit!==build.source_commit || object(current.metadata).source_tree!==build.source_tree
      || !authorityAcquiredAfterStart(current.acquired_at,observed.process_started_at))return fail()
