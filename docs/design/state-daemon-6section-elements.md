...
- `last_wake_attempt_at` 更新前に wake 実行する path は禁止 (重複 wake 抑制が壊れる)
...
- **F6**: 既存 `bin/state-daemon.ts` を残したまま state-daemon を起動しないこと。phase 5 で state-daemon を停止、それまでは並行稼働 (phase 3-4) でも **重複 wake は許容、prevention で抑制しない** (idempotent + duplicate suppression で吸収)
