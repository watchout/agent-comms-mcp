...
The historical `regression1-final`/`regression2-final` snapshots omitted the then-untracked `core/process-start-time.ts`. The two primary suites were therefore rerun from the fixed commit as `regression1-candidate`/`regression2-candidate`; these and `integrated-final` have complete source snapshots, with zero missing or mismatched candidate production inputs and invoked tests/helpers. Historical snapshots and failures remain unmodified.
...
| `regression1-candidate` | **85 PASS / 57 FAIL / 419 assertions**, 142 tests | Required heartbeat/seat/endpoint/current/scheduler/queue suite. Physical fixture inserts reject and old observer injection paths fail; scheduler timeouts remain. This is unresolved regression evidence. |
| `regression2-candidate` | **33 PASS / 26 FAIL / 130 assertions**, 59 tests | Required readiness/native/configuration suites. Old physical fixture inserts and two PostgreSQL desired-state cases fail. Owned PostgreSQL URL is now valid; these are not credited as environment setup errors. |
...
The default Node executable became unable to load `libsimdjson.30.dylib` during this window. That raw failure remains in `native-cleanup-proof.log`. Testing an already installed Node 24.20.0 succeeded; only the private test runner PATH changed. No Homebrew/package/system configuration repair was performed. [fixture-stop-final.json](independent/fixture-stop-final.json) records owned PostgreSQL shutdown and the bounded fixture-process check.
...
Fixture restart evidence: the first restart omitted the original command-line socket-only options and failed to bind the occupied default TCP port; no server started (`fixture-restart01.log`). Explicit empty listen-addresses and the owned socket path restored the private fixture (`fixture-restart02.log`). No other process was stopped or reconfigured. The two fixed-commit regression runs completed, and `fixture-stop-final.json` confirms final owned shutdown.
