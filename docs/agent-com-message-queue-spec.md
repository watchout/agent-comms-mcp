...
- **timeout resolver の一本化(L2 cycle-3 F1)**: runner child の実行 timeout と daemon の slot-wedge 予算は `core/queue-work-timeout.ts` の**単一 resolver** から導出する(構造的に乖離不能)。canonical precedence は「engine 固有 > generic、各層内で AUN > STATE」: `AUN_<ENGINE>_TIMEOUT_MS > STATE_DAEMON_<ENGINE>_TIMEOUT_MS > AUN_QUEUE_WORK_TIMEOUT_MS > STATE_DAEMON_QUEUE_WORK_TIMEOUT_MS > 600000`。slot 予算 = 全 engine 解決値の最大。旧 Claude 経路の「generic AUN が engine STATE に勝つ」逆転は本 normalization で廃止。malformed/非正値は両側で typed error(daemon = DB 接続前に起動失敗、runner = QUEUE_WORK_TIMEOUT_CONFIG_INVALID の typed 失敗)。unset は default 600000(明文 fallback)
