...
- Automatic processing requires all of the following DB-derived conditions: the agent is enrolled, `profile_enabled` is true, `disabled_at` is null, a non-empty ready runtime and non-empty status are configured, the status is not disabled/offline/retired, the target agent is in the message channel's `channels.members`, and the agent is not `agent_type='human'`. The former `STATE_DAEMON_AGENT_DENYLIST` is retired (Issue #940 fix 3): eligibility never depends on a file/env name list, and readiness reports a leftover denylist key as configuration drift.
